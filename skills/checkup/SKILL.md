...
- [ ] Compare the index timestamp at `~/.dont-forget/index.db` with the newest vault note modification. If the index is missing or older, run `python3 scripts/index.py` before producing the report.
...
- [ ] Run `python3 scripts/checkup.py` and explain the result plainly: total notes, orphan notes, unresolved-link candidates, and stale notes.
